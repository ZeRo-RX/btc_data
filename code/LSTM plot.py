...
x_train, y_train = [], []
w = 30
for i in range(w, len(df)):  # increase the number of days used for training
    x_train.append(df[i-w:i, 0])  # use 120 days of data for training
...
# Train the model
model.fit(x_train, y_train, batch_size=1, epochs=10, validation_split=0.4, callbacks=[early_stop])  # increase the number of epochs and add validation split and early stopping
...
for i in range(num_days):
    last_120_days = df[-w:]  # use the last 120 days of data for prediction
...
ds = pd.read_csv('BTC-USD (1).csv') 
plt.figure(figsize=(10, 6))
plt.plot(dates, ds['High'], label='Actual TMin')
plt.plot(predicted_dates, predicted_prices, label='Predicted TMin')
plt.legend()
plt.xlabel('Date')
plt.ylabel('TMin')
